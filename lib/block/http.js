...
module.exports = inherit(Block, {
    ...
    _doHttp : function(params, promise, dataType, bodyData, redirCounter) {
        var t = this;
        t._curReq = http.request(
                ...
                function(res) {
                    if(res.statusCode === 301 || res.statusCode === 302) {
                        return --redirCounter?
                            t._doHttp(url.parse(res.headers['location'], true), promise, dataType, null, redirCounter) :
                            promise.reject(buildHttpError({ message : 'too many redirects' }));
                    }
                    else if(res.statusCode >= 400) {
                        return promise.reject(buildHttpError({ code : res.statusCode }));
                    }
                    ...
                    res
                        ...
                        .once('end', function() {
                            try {
                                promise.fulfill(processResponse(body, dataType || 'json'));
                            }
                            catch(e) {
                                promise.reject(buildHttpError({ message : 'parse error: ' + e.message }));
                            }
                        })
                        .once('close', function() {
                            promise.reject(buildHttpError({ message : 'connection closed' }));
                        });
                });
        ...
        t._curReq
            .once('error', function(e) {
                promise.reject(buildHttpError({ message : e.message }));
            })
            .end();
    }
});
