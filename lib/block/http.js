...
module.exports = inherit(Block, {
    run : function(params, promise) {
        ...
        if(params.allowCompression) {
            ...
            if(!encodingHeader) {
                encodingHeader = COMPRESSION_METHODS.join(', ') + ', *';
            }
            else {
                COMPRESSION_METHODS.forEach(function(method) {
                    encodingHeader.indexOf(method) === -1 && (encodingHeader = method + ', ' + encodingHeader);
                });
            }
    ...
    _doHttp : function(params, promise, dataType, body) {
        ...
        _this._curReq = (params.protocol === 'https:'? https : http).request(
                ...
                function(res) {
                    ...
                    COMPRESSION_METHODS.indexOf(headers['content-encoding']) > -1?
                        res.pipe(resStream = new zlib.Unzip()) :
                        resStream = res;
