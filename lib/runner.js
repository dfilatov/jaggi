...
module.exports = inherit({
    ...
    _runBlock : function(blockId, block) {
        ...
        if(typeof block.call === 'object') {
            promise = t._runSubRunner(block.call, blockParams, blockPath);
        }
        ...
        if(runObj) {
            ...
            promise || (promise = t._runSubRunner(blockContentRes, blockParams, blockPath));
        }
    ...
    _runSubRunner : function(blocks, params, path) {
        return new this.__self(
            blocks,
            utils.merge(this._ctx.params(), { state : new State(params) }),
            utils.merge(this._params, { path : path }))
                .run();
    },
