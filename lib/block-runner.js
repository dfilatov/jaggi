...
module.exports = inherit({
    ...
    _onBlockFailed : function(blockError) {
        ...
        if(this._blockError.message === 'timed out') { // надо как-то получше отличать таймаут от реального режекта
            this.abort();
    ...
    _processInclude : function(desc, params) {
        ...
        while(desc.include) {
            ...
            if(!--depth) {
                throw({
                    message : 'includes depth (' + params.maxIncludeDepth + ') reached in file: ' + fileName
                });
            }
        }
